...
public class LoginInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object arg2) throws Exception {
        ...
        //指定不拦截的路径
        final String[] IGNORE_URI = {"/register","/login", "/home","/user/" };

        //指定拦截的路径，需要登录才可以进入的页面
        final String[] PRE_UNIGNORE_URI = {
                "/user/index","/user/addUser","/user/getAllUser",
        ...
        if (uri.startsWith("/user/registerjump") || uri.startsWith("/user/loginjump") || uri.startsWith("/register")|| uri.startsWith("/login")) {
            return true;
        }
